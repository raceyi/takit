...
export class ShopCartPage{
    ...
    totalAmount:number=0;   //total amount. acutual price with discount
    cart:any={};
    ...
    orderInProgress=false;
     ...
     constructor(private app:App,private navController: NavController
        ...
        if(this.cart!=undefined){
            ...
            this.totalAmount=this.price-this.discount;
            if(this.delivery==true && this.totalAmount<this.storageProvider.shopInfo.freeDelivery){
                this.delivery=false;
                this.takeout=false;
                this.here=true;
            }
    ...
    cashPasswordFocus(){
        console.log("cashPasswordFocus-dimensions:"+JSON.stringify(this.cartPageRef.getContentDimensions()));
        this.cartPageRef.scrollToBottom();
    ...
    ionViewWillEnter(){
     ...
     order(){
         if(!this.orderInProgress){
                ...
                if(this.storageProvider.cashAmount<this.totalAmount){
                    let alert = this.alertController.create({
    ...
    deleteAll(){
     ...
     onFocusPassword(event){
         if(!this.storageProvider.isAndroid){
            ...
            let dimensions = this.cartPageRef.getContentDimensions();
            console.log("dimensions:"+JSON.stringify(dimensions));
            this.cartPageRef.scrollTo(0, dimensions.contentHeight);
         }
    }
...
changeTakeout(takeoutOption:number){
    ...
    increaseCount(menu){
        menu.quantity++;
        this.totalAmount+=parseInt(menu.price)-Math.round(menu.price*(parseFloat(this.storageProvider.shopInfo.discountRate)/100.0));
    }
